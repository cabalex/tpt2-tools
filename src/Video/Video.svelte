<script lang="ts">
  ...
  async function convert() {
    processing = true;
    frames = (await processVideo(file, aspectRatio, step)).map((f) => {
      return { url: f, downloaded: false };
    });
    processing = false;
  }
...
<p class="intro" style="max-width: 700px">
  <b>Play videos in TPT2 using a sequencer!</b> Simply create an image panel and
  attach it to a sequencer. Then, use the decals generated here to create your
  video! (you must upload each one manually first.)
  <br />
</p>
<DecalSizer bind:aspectRatio />
<UploadHandler accept="video/mp4" bind:file />
<div>
  FPS: <input
    value={fps}
    on:change={(e) => {
  ...
  <br />
  <i
    >The fastest framerate sequencers can support is 1 image per 0.25s per image
    panel (4 fps).</i
  >
</div>
<button
  ...
  <div class="frames">
    ...
    {#each frames as frame, i}
      <div class="frame" class:downloaded={frame.downloaded}>
        ...
        {#if frame.asset}
          {#if frame.asset.assetId}
            <button
              class="copyBtn"
              on:click={() => {
                navigator.clipboard.writeText(frame.asset.assetId);
          ...
          {:else}
            <span style="margin-right: 10px">{frame.asset.status}</span>
          {/if}
        {/if}
...
<style>
  ...
  :global(.tpt2tools .frame img) {
    background-image: linear-gradient(45deg, #444 25%, transparent 25%),
      ...
      linear-gradient(-45deg, transparent 75%, #444 75%) !important;
  }
</style>
